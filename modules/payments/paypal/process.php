...
{
    $itemInfoJson = urldecode($_POST['itemInfo']);         //contains all info from all items, json formatted
    echo $itemInfoJson;
    $itemInfo = json_decode($itemInfoJson, true);     //decode the json
    echo $itemInfo."<br>";
    //$numItems = $_POST['numItems'];             //the number of different items
    $ItemTotalPrice = $_POST['totalPrice'];     //total price calculated
    $itemCount = 0;
    ...
    foreach ($itemInfo as $product_id => $item){
        /*
        $padata .=  '&L_PAYMENTREQUEST_0_QTY0='. urlencode($ItemQty).
                    '&L_PAYMENTREQUEST_0_AMT0='.urlencode($ItemPrice).
                    '&L_PAYMENTREQUEST_0_NAME0='.urlencode($ItemName).
                    '&L_PAYMENTREQUEST_0_NUMBER0='.urlencode($ItemNumber);
         * 
         */
        $padata .=  '&L_PAYMENTREQUEST_0_QTY'.$itemCount.'='. urlencode($item['quantity']).
                    '&L_PAYMENTREQUEST_0_AMT'.$itemCount.'='.urlencode($item['price']).
                    '&L_PAYMENTREQUEST_0_NAME'.$itemCount.'='.urlencode($item['product_name']).
                    '&L_PAYMENTREQUEST_0_NUMBER'.$itemCount.'='.urlencode($product_id);
        $itemCount ++ ;
    }
    echo $padata;
